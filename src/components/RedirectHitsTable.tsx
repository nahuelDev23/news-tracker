import type { RedirectHitRecord } from "@/types/redirect";
import { formatHitDate } from "@/lib/format-date";

interface RedirectHitsTableProps {
  hits: RedirectHitRecord[];
}
          ...
          {hits.map((hit) => (
            <tr key={hit.id} className="hover:bg-slate-900/50">
              ...
              <td className={`${tdClass} text-slate-300`}>
                ...
                <div className="truncate text-xs text-slate-500">{hit.isp ?? "—"}</div>
              </td>
              <td className={`${tdClass} text-slate-300`}>
